...
# import os # Baris ini sudah dihapus
    ...
    if st.session_state.kproto_model is None or st.session_state.scaler is None:
        ...
        if submitted:
            if input_rata_nilai is None or input_kehadiran is None:
                ...
                # --- FIX Applied Here ---
                bars = sns.barplot(x=labels_for_plot, y=values_for_plot, hue=labels_for_plot, palette="viridis", legend=False, ax=ax)
        ...
        if df_for_visual_clustering is not None:
            ...
            for i in range(k_visual):
                ...
                with col2:
                    ...
                    fig, ax = plt.subplots(figsize=(10, 6))
                    # --- FIX Applied Here ---
                    bars = sns.barplot(x=labels_for_plot, y=values_for_plot, hue=labels_for_plot, palette="cubehelix", legend=False, ax=ax)
    ...
    if st.session_state.df_clustered is None or st.session_state.df_original is None or st.session_state.df_original.empty:
        ...
        if nama_terpilih:
            ...
            with col_chart:
                ...
                fig, ax = plt.subplots(figsize=(10, 6))
                # --- FIX Applied Here ---
                bars = sns.barplot(x=labels_siswa_plot, y=values_siswa_plot, hue=labels_siswa_plot, palette="magma", legend=False, ax=ax)
